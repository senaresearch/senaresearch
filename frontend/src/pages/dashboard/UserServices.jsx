...
const UserServices = () => {
    ...
    const service_update = async (event)=>{
        ...
        formData.append('category', JSON.stringify(selectedService?.category))
    ...
    <div className='font-[Montserrat-Arabic] mt-16 lg:p-4 mx-auto lg:w-9/12 h-fit
        ...
        <Modal trigger={editModal} setTrigger={setEditModal}  header={' تحـــديث الخدمة'} modalWidth={'1000px'}>
            <form method='POST' enctype="multipart/form-data" onSubmit={service_update} className='flex-col gap-6 px-4 mx-auto items-center md:flex-row md:flex-wrap md:justify-center lg:grid grid-cols-12'>
                <input required value={selectedService?.name} onChange={(e)=>{setSelectedService(prevState=>({...prevState, name:e.target.value}))}} className='w-full lg:col-span-12 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8] ' type="text" placeholder='اسم الخدمة' />
                <select required onChange={(e)=>{setSelectedService((prevState)=>({...prevState, category: parseInt(e.target.value)}))}} name="category" id="category" className='w-full lg:col-span-6 placeholder:text-sm text-primary placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]'>
                    <option value="">نـوع الخدمة</option>
                    <option selected value={selectedService?.category.id}>{selectedService?.category.name}</option>
                    {categories && categories.map((category)=>(
                        (selectedService?.category.id !== category.id) && 
                        <option value={category?.id} key={category.id} className=' px-6 py-7 '>{category?.name}</option>
                    ))}
                </select>
                <input required value={selectedService?.price} onChange={(e)=>setSelectedService(prevState=>({...prevState, price:e.target.value}))} className='w-full lg:col-span-6 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' type="text" placeholder='االسعر ' />
                <textarea required value={selectedService?.description} onChange={(e)=>setSelectedService(prevState=>({...prevState, description:e.target.value}))} className='w-full lg:col-span-full placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' placeholder='وصف الخدمة' name="" id="" cols="30" rows="10"></textarea>
                <input required={!selectedService?.image && true} onChange={(event)=>{ setSelectedService(prevState=>({...prevState, image:''}));setFile(event.target.files[0])}} className='col-span-full text-left placeholder:text-sm placeholder:text-primary placeholder:leading-4 px-2 py-2 rounded-lg bg-[#F1F3F8]' type="file" accept="image/*" placeholder='اختـر صورة ' />                
                {
                    ...
                    <div className='col-span-full'>
                        <div className='w-full h-auto relative'>
                            ...
                            <img className='w-full h-auto rounded-lg shadow-xl' src={`${selectedService?.image ? 'https://sena-reserach.vercel.app/'+selectedService?.image : file && URL.createObjectURL(file)}`} alt='service'/>
                        </div>
                    </div>
        ...
        <Modal trigger={newServiceModal} setTrigger={setNewServiceModal} header={'إضـافة خدمـة جديـدة'} modalWidth={'1000px'}>
            <form enctype="multipart/form-data" onSubmit={service_create} className='flex-col gap-6 px-4 mx-auto items-center md:flex-row md:flex-wrap md:justify-center lg:grid grid-cols-12'>
                <input required value={selectedService?.name} onChange={(e)=>setSelectedService(prevState=>({...prevState, name:e.target.value}))} className='w-full lg:col-span-12 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8] ' type="text" placeholder='اسم الخدمة' />
                <select required onChange={(e)=>{setSelectedService(prevState=>({...prevState, category:{...prevState?.category, id:e.target.value}})); console.log(selectedService)}} name="category" id="category" className='w-full lg:col-span-6 placeholder:text-sm text-primary placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]'>
                    <option value="" disabled selected>نـوع الخدمة</option>
                    {categories && categories.map((category)=>(
                        <option value={category?.id} key={category.id} className=' px-6 py-7 '>{category?.name}</option>
                    ))}
                </select>
                <input required value={selectedService?.price} onChange={(e)=>setSelectedService(prevState=>({...prevState, price:e.target.value}))} className='w-full lg:col-span-6 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' type="number" placeholder='االسعر ' />
                <textarea required value={selectedService?.description} onChange={(e)=>setSelectedService(prevState=>({...prevState, description:e.target.value}))} className='w-full lg:col-span-full placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' placeholder='وصف الخدمة' name="" id="" cols="30" rows="10"></textarea>
                <input required onChange={(event)=>setFile(event.target.files[0])} className='col-span-full text-left placeholder:text-sm placeholder:text-primary placeholder:leading-4 px-2 py-2 rounded-lg bg-[#F1F3F8]' type="file" accept="image/*" placeholder='اختـر صورة ' />
                {file && 
                    <div className='col-span-full'>
                        <div className='w-full h-auto relative'>
                            <span onClick={(e)=>{setSelectedService(prevState=>({...prevState, image:''})); setFile(null); e.target.parentElement.parentElement.parentElement.previousElementSibling.value=''}} className=' absolute p-1 rounded-full hover:bg-gray-300 shadow-md transition ease-in-out cursor-pointer right-1 top-1 duration-700'>
