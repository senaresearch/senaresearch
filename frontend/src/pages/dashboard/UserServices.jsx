                ...
                <input required={!selectedService?.image && true} onChange={(event)=>{ setSelectedService(prevState=>({...prevState, image:''}));setFile(event.target.files[0])}} className='col-span-full text-left placeholder:text-sm placeholder:text-primary placeholder:leading-4 px-2 py-2 rounded-lg bg-[#F1F3F8]' type="file" accept="image/*" placeholder='اختـر صورة ' />                
                {
                    ...
                    <div className='col-span-full'>
                        <div className='w-full h-auto relative'>
                            ...
                            <img className='w-full h-auto rounded-lg shadow-xl' src={`${selectedService?.image ? 'http://127.0.0.1:8000/'+selectedService?.image : file && URL.createObjectURL(file)}`} alt='service'/>
                        </div>
                    </div>
