...
//         formData.append('category', selectedService?.category.name)
...
//                                 <tr onClick={()=>{setSelectedService(service)}} key={service?.id} class="bg-white transition duration-300 ease-in-out cursor-pointer border-b hover:bg-gray-50">
...
//                                         {service?.name}
//                                     </th>
//                                     <td class="whitespace-nowrap px-6 py-4 font-medium text-center">{index}</td>
//                                 </tr>
//                                 )
//                             )
//                         }
...
//             </div>
//         </Modal>
//         <Modal trigger={editModal} setTrigger={setEditModal} mainAction={"تحـــديث"}  header={' تحـــديث الخدمة'}modalWidth={'1000px'}>
...
//                 <input value={selectedService?.name} onChange={(e)=>setSelectedService(prevService=>({...prevService, name:e.target.value}))} className='w-full lg:col-span-12 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8] ' type="text" placeholder='اسم الخدمة' />
//                 <input value={selectedService?.category?.name} onChange={(e)=>setSelectedService(prevService=>({...prevService, category:{...prevService?.category, name:e.target.value}}))} className='w-full lg:col-span-6 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8] ' type="text" placeholder='نوع الخدمة' />
//                 <input value={selectedService?.price} onChange={(e)=>setSelectedService(prevService=>({...prevService, price:e.target.value}))} className='w-full lg:col-span-6 placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' type="text" placeholder='االسعر ' />
//                 <textarea value={selectedService?.description} onChange={(e)=>setSelectedService(prevService=>({...prevService, description:e.target.value}))} className='w-full lg:col-span-full placeholder:text-sm placeholder:text-primary placeholder:leading-4 text-right px-2 py-2 rounded-lg bg-[#F1F3F8]' placeholder='وصف الخدمة' name="" id="" cols="30" rows="10"></textarea>
...
//                             <span onClick={(e)=>{e.target.parentElement.parentElement.parentElement.previousElementSibling.value=''}} className=' absolute p-1 rounded-full hover:bg-gray-300 shadow-md transition ease-in-out cursor-pointer right-1 top-1 duration-700'>
